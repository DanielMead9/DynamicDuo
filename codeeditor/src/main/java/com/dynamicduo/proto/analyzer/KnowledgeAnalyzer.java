package com.dynamicduo.proto.analyze;
 ...
 *   - When P sends or receives a message, P "sees" the terms that appear in the clear.
 ...
 *   - If P knows Enc(k, m) and also knows k (from somewhere else), then we add m to P's knowledge.
 *   - We repeat that inference until no new terms are learned.
 *
 * Simplifications:
 *   - Identifiers (IdentifierNode) are treated as atomic symbols: "K_AB", "N_A", "M_1", "c", etc.
 *   - EncryptExprNode is treated as a term Enc(key, message).
 *   - "Catastrophic" is: the adversary learns any K_* or M_* (keys or plaintext messages),
